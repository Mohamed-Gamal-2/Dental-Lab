...
const getAllManagers = async (req, res) =>{
  try {
    ...
    return res.status(200).json({ message: "managers retrieved successfully", allManagers });
  } catch (err) {
...
const getOneManager = async (req, res) =>{
  try {
    ...
    return res.status(200).json({message:"manager retrieved successfully", oneManager})
  } catch (err) {
    console.log("catch error: ",err);
    return res.status(400).json({ message: "catch error: ",err });
  }
...
export { getAllManagers, getOneManager, addManager, signInManager, updateManager, deleteManager };
